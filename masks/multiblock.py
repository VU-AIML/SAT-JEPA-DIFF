...
class MaskCollator(object):
    ...
    def _sample_block_size(self, generator, scale, aspect_ratio_scale):
        """
        Sample a block size (h, w) in patch units, making sure that
        h * w > min_keep and h, w are within [1, height-1] / [1, width-1].
        This avoids impossible masks on small grids (e.g. 8x8).
        """
        for _ in range(50):
            ...
            # Make sure there is at least (min_keep + 1) patches possible
            max_keep = max(max_keep, self.min_keep + 1)
            ...
            # Clip to valid range and avoid zero
        ...
        # Fallback: reasonably sized block in the middle of the grid
        h = max(1, min(self.height - 1, self.height // 2))
        w = max(1, min(self.width - 1, self.width // 2))
        return (h, w)
        ...
        while not valid_mask:
            ...
            if not valid_mask:
                ...
                if timeout == 0:
                    ...
                    timeout = og_timeout
                    logger.warning(
                        'Mask generator says: "Valid mask not found, decreasing acceptable-regions [%d]"'
                        % tries
                    )
        mask = mask.squeeze()
        # --
